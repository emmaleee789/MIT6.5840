...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("Server %d is leader, now disconnect it...\n", leader1)

	// if the leader disconnects, a new one should be elected.
	DPrintf("......if the leader disconnects, a new one should be elected......")
	cfg.disconnect(leader1)
	cfg.checkOneLeaderParam(1)

	// if the old leader rejoins, that shouldn't
	// disturb the new leader. and the old leader
	// should switch to follower.
	DPrintf("......if the old leader rejoins, not disturbing......")
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeaderParam(2)

	// if there's no quorum, no new leader should
	// be elected.
	DPrintf("......if there's no quorum, no new leader should......")
